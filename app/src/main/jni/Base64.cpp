...
unsigned char *base64_encode(unsigned char *str) {
    ...
    //计算经过base64编码后的字符串长度
    str_len = (long) strlen((const char *) str);
    if (str_len % 3 == 0)
        len = str_len / 3 * 4;
    else
        len = (str_len / 3 + 1) * 4;
    ...
    for (i = 0, j = 0; i < len - 2; j += 3, i += 4) {
        ...
        res[i + 1] = base64_table[
                (str[j] & 0x3) << 4 | (str[j + 1] >> 4)]; //将第一个字符的后位与第二个字符的前4位进行组合并找到对应的结果字符
        res[i + 2] = base64_table[
                (str[j + 1] & 0xf) << 2 | (str[j + 2] >> 6)]; //将第二个字符的后4位与第三个字符的前2位组合并找出对应的结果字符
...
unsigned char *base64_decode(unsigned char *code) {
    ...
    int i, j;
    //计算解码后的字符串长度
    len = (long) strlen((const char *) code);
    //判断编码后的字符串后是否有=
    ...
    //以4个字符为一位进行解码
    for (i = 0, j = 0; i < len - 2; j += 3, i += 4) {
        res[j] = ((unsigned char) table[code[i]]) << 2 | (((unsigned char) table[code[i + 1]])
        ...
        res[j + 2] = (((unsigned char) table[code[i + 2]])
                << 6) | ((unsigned char) table[code[i + 3]]); //取出第三个字符对应base64表的十进制数的后2位与第4个字符进行组合
    }
    return res;
}